...
`LogisticSolver::Lbfgs` is the default. It never forms a second-order system:
its storage is linear in the parameter count, so it accepts far larger
multiclass problems, and it is the only path that answers when the exact one
refuses with `ModelError::MulticlassSystemTooLarge`.

`LogisticSolver::Newton` takes the exact second-order step and converges in
single-digit iterations. Its cost is one `parameters x parameters`
factorization per iteration *plus* accumulating that system over every row, and
a joint multinomial system is `classes * parameters` square — so on a wide
multiclass problem it refuses rather than allocate something it cannot hold.
Select it on small data, where the single-digit iteration count wins outright,
and when `tol` is below the matrix-free path's floor.

**The default used to be `Newton`, and it moved on measurement.** The case for
keeping it rested on 50,000 rows by 50 columns, same data, same `tol`, same
...
read at the time as an advantage belonging to one large shape rather than a
reason to move the default. A wider sweep, four shapes by three seeds by two
task families, run twice on identically generated bytes, says otherwise:
**2.4x to 50.6x across 24 measurements, always in the matrix-free path's
favour**, with 50.6x at `1024 x 128` multiclass and 47.1x at `4096 x 128`. The
distance from the recorded truth is identical to five decimal places in 23 of
the 24, and both FerricML solvers and the external reference land on the same
truth-distance to five decimals at every point on the grid: the solver choice is
not visible in the answer.

`max_iter` is not the binding constraint — raising it from 200 to 1000 moved the
exact fit's time by under 1%. The mechanism is the exact step's factorization
scaling with the class count, which is why the multiclass rows are so much worse
than the binary ones: at `4096 x 128`, binary is 25.2 ms and four-class is
344.7 ms.
...
- **`tol` measures a different quantity per solver, so the default meaning of
  `tol` moved with the default solver.** Under `Newton` it is the largest
  absolute coefficient update; under `Lbfgs` it is the gradient norm of the mean
  penalized objective. The default *value* is unchanged at `1e-4`; the quantity
  it bounds is not. They are documented apart rather than conflated, because
  pretending they were the same would make one of them wrong.
- **`Lbfgs` has a tolerance floor and `Newton` does not.** Below roughly `1e-9`
  for a log-loss of order one, the matrix-free line search cannot bracket a
  decrease and the fit reports `ModelError::SolverDidNotConverge`. A caller who
  needs a tolerance under that floor should select `Newton` explicitly.
