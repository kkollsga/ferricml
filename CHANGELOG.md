...
- **The default logistic solver is now `LogisticSolver::Lbfgs`.** It was
  `Newton`, and the documented case for keeping it rested on one shape — 50,000
  rows by 50 columns, where the matrix-free path is 3x to 15x faster. A wider
  sweep, four shapes by three seeds by two task families, run twice on
  identically generated bytes, measured **2.4x to 50.6x across 24 measurements,
  always in the matrix-free path's favour**: 50.6x at `1024 x 128` multiclass,
  47.1x at `4096 x 128`, and 2.4x at the smallest binary shape. The distance
  from the recorded truth is identical to five decimal places in 23 of the 24
  and differs in the fifth decimal in the other. `max_iter` is not the binding
  constraint — raising it from 200 to 1000 moved the exact fit's time by under
  ...
  **The meaning of the default `tol` changed with it.** The two solvers have
  ...
  gradient of the mean penalized objective. The default *value* is unchanged at
  `1e-4` — the quantity it bounds is not. `Lbfgs` also has a floor the exact path
  does not: below roughly `1e-9` for a log-loss of order one its line search
  cannot bracket a decrease and the fit reports
  `ModelError::SolverDidNotConverge`. **A caller who needs a tolerance under that
  floor, or who depends on the exact path's iterate sequence, must now select
  `LogisticSolver::Newton` explicitly.**

  Anything fitting through `LogisticRegressionParams::default()` moves with it,
  including `PairwiseLinearRanker`, whose coefficients shift by about `1.2e-4`
  relative — inside the `1e-4` tolerance the fit was asked for rather than a
  different answer, and now asserted against both solvers rather than pinned to
  one number. `ModelError::MulticlassSystemTooLarge` is a property of the
  *selected* solver, so a wide multiclass fit that used to refuse under the
  default now succeeds.
